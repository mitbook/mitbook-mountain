...
public class MitGlobalTransactionManager {
    ...
    public Integer calChildTransactionStatus(String globalTransactionId) {
        ...
        while (iterator.hasNext()) {
            ...
            if (childTransaction.getTransationalEnumStatusCode() == TransactionalEnumStatus.RollBACK.getCode()) {
                needRoolBack = true;
            }
            //统计commit的个数
            if (childTransaction.getTransationalEnumStatusCode() == TransactionalEnumStatus.COMMIT.getCode()) {
                commitCount++;
            }

            if (childTransaction.getTransactionalTypeEunmCode() == TransactionalTypeEnum.BEGIN.getCode() ||
                    childTransaction.getTransactionalTypeEunmCode() == TransactionalTypeEnum.END.getCode()) {
                beginAndEnd++;
            }
